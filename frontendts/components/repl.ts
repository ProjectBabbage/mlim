...
export default function REPL() {
  ...
  return (
    <div>
      <div>
        <div>Available vars ( use STATE["var_name"] )</div>
        {varsDisplay}
      </div>
      <div>
      ...
      <div
        className={`
            h-96 w-96 whitespace-pre-line
            ${toggled ? "border border-slate-500 " : " "} 
            `}
        onClick={() => toggleREPL()}
      ...
      >
        ...
        {">"}
        {content}
        {toggled ? <div className="animate-pulse inline">|</div> : ""}
      </div>
    </div>
  );
}
